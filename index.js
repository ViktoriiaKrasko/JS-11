...
function customSet(arr) {
  const stringSet = new Set(arr); // Створення множини з масиву за допомогою конструктора Set
  for (const value of stringSet) {
    if (typeof value === "number") {
      stringSet.delete(value);
    }
  }
  return stringSet;
...
function addElements(mySet, arr) {
  ...
  return mySet; // Перебираємо масив та додаємо кожний елемент до множини за допомогою методу add
...
function setToArray(mySet) {
  const myArray = [...mySet]; // Конвертуємо множину в масив за допомогою деструктурізації.
  const stringArray = myArray.filter((el) => typeof el === "string");
  stringArray.sort();
  return stringArray; // Використовуємо метод filter для створення нового масиву, що містить лише рядкові елементи.
...
function removeDuplicatesInPlace(arr) {
  const mySet = new Set(arr); // Створення множини для збереження унікальних елементів
...
function areDisjoint(set1, set2) {
  ...
  return true; // Перебираємо першу множину за допомогою оператору for of
...
function getIntersection(arr1, arr2) {
  ...
  const set2 = new Set(arr2); // Створення множин з двох масивів
...
function iterateSet(set) {
  const keys = set.keys(); // Використовуємо методу keys для отримання ітератора ключів
